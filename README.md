...
→ inherited from parent process (AGENTCTX_CONTEXT) → global default (csw use) → none
```
...
`inherited` is what makes agents composable: `csw run --context acme -- claude` exports `AGENTCTX_CONTEXT=acme`, so when the agent itself calls `csw run -- az ...` anywhere on disk, it stays acme unless a folder binding says otherwise.
